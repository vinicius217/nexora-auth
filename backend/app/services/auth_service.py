...
from backend.app.schemas.usuario import LoginRequest, Token, UsuarioCreate
...
class AuthService:
    ...
    def registrar(self, data: UsuarioCreate) -> tuple[Usuario, str]:
        ...
        if len(set(data.senha)) < 4:
            raise HTTPException(400, "Escolha uma senha mais forte.")
        if self.repository.get_by_email(data.email):
            raise HTTPException(400, "Já existe um usuário cadastrado com esse e-mail.")
    ...
    def login(self, data: LoginRequest) -> tuple[Token, str]:
        ...
        invalid = HTTPException(
            401, "E-mail ou senha inválidos.", headers={"WWW-Authenticate": "Bearer"}
        )
