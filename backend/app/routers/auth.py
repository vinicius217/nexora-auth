...
from backend.app.models.usuario import Sessao, Usuario
...
def create_session(
    db: Session, usuario: Usuario, request: Request
) -> tuple[str, Sessao]:
...
def registrar(
    data: UsuarioCreate, request: Request, service: AuthService = Depends(get_service)
):
    ...
    return {
        "usuario": usuario,
        "dev_verification_token": token if settings.EMAIL_DEV_MODE else None,
...
def login(
    ...
    refresh_age = (
        settings.REFRESH_TOKEN_EXPIRE_DAYS * 86400 if data.lembrar_me else None
    )
    set_auth_cookie(response, "access_token", token.access_token, access_age)
    set_auth_cookie(response, "refresh_token", refresh, refresh_age)
    return token
...
def demo_login(request: Request, response: Response, db: Session = Depends(get_db)):
    ...
    set_auth_cookie(
        response, "access_token", access, settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60
    )
    set_auth_cookie(
        response, "refresh_token", refresh, settings.REFRESH_TOKEN_EXPIRE_DAYS * 86400
    )
...
def verificar_email(token: str, db: Session = Depends(get_db)):
    usuario = (
        db.query(Usuario)
        .filter(Usuario.verificacao_token_hash == hash_token(token))
        .first()
    )
    if (
        not usuario
        or not usuario.verificacao_token_expira_em
        or usuario.verificacao_token_expira_em
...
def reenviar_verificacao(
    ...
    try:
        ...
        raise HTTPException(
            502, "Não foi possível enviar o e-mail. Tente novamente em instantes."
        ) from exc
    if not enviado:
...
def refresh(request: Request, response: Response, db: Session = Depends(get_db)):
    ...
    sessao = (
        db.query(Sessao)
        .filter(
            Sessao.id == payload.get("sid"),
            Sessao.refresh_jti_hash == hash_token(payload.get("jti", "")),
    ...
    user = (
        db.query(Usuario)
        .filter(Usuario.email == payload.get("sub"), Usuario.id == sessao.usuario_id)
        .first()
    )
    ...
    access = criar_access_token({"sub": user.email})
    set_auth_cookie(
        response, "access_token", access, settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60
    )
    set_auth_cookie(
...
def logout(request: Request, response: Response, db: Session = Depends(get_db)):
    ...
    if payload and payload.get("sid"):
        ...
        if sessao:
            sessao.revogada_em = datetime.now(timezone.utc).replace(tzinfo=None)
            db.commit()
...
def eu(usuario_atual: Usuario = Depends(get_current_user)):
...
def atualizar_perfil(
    ...
    usuario.nome = " ".join(data.nome.strip().split())
    usuario.avatar_url = str(data.avatar_url) if data.avatar_url else None
    db.commit()
    db.refresh(usuario)
    return usuario
...
def alterar_senha(
    ...
    db.query(Sessao).filter(
        Sessao.usuario_id == usuario.id, Sessao.revogada_em.is_(None)
    ).update({"revogada_em": datetime.now(timezone.utc).replace(tzinfo=None)})
    db.commit()
    return {
        "message": "Senha alterada com sucesso. Entre novamente nos seus dispositivos."
    }
...
def esqueci_senha(
    ...
    if is_demo_user(usuario):
        return {
            "message": "A conta de demonstração é restaurada automaticamente e não possui senha pública."
        }
    token = gerar_token_aleatorio()
    usuario.reset_token_hash = hash_token(token)
    ...
    if settings.EMAIL_DEV_MODE:
        resposta["message"] = (
            "Solicitação criada. Use o código de desenvolvimento retornado abaixo."
        )
...
def resetar_senha(data: ResetPasswordRequest, db: Session = Depends(get_db)):
    ...
    if (
        not usuario
        or not usuario.reset_token_expira_em
        or usuario.reset_token_expira_em < agora
    ):
