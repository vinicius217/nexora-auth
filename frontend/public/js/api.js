...
function setLoading(btn, loading, label="Processando...") { btn.disabled=loading; if(btn.classList.contains("react-3d-button")){if(!btn.dataset.label) btn.dataset.label=btn.getAttribute("aria-label")||"Entrar na conta";btn.classList.toggle("is-loading",loading);btn.setAttribute("aria-busy",String(loading));btn.setAttribute("aria-label",loading?label:btn.dataset.label);return} if(loading){btn.dataset.label=btn.textContent;btn.innerHTML='<span class="spinner"></span>'+label}else btn.textContent=btn.dataset.label||btn.textContent; }
