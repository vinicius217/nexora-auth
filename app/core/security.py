...
def criar_token(dados: dict, minutos: int, tipo: str) -> str:
    payload = dados.copy(); payload.update({"exp": datetime.now(timezone.utc)+timedelta(minutes=minutos), "type": tipo, "jti": secrets.token_hex(16)})
...
def criar_refresh_token(dados: dict) -> str:
    return criar_token(dados, settings.REFRESH_TOKEN_EXPIRE_DAYS*24*60, "refresh")
